...
// use stack to manage physical memory
...
void init_pmm() {
    ...
    for(map_entry = mmap_start_addr; map_entry < mmap_end_addr; map_entry++) {
        if(map_entry->type == 1 && map_entry->base_addr_low == 0x100000) {              // 1 means avaliable memory

            // save memory from kernel end to memory end as pages
            // support 512MB physical memory at most
